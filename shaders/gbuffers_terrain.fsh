...
void main(){
    ...
    if(lmcoord.y > 0.01){
        ...
        shadowVis = shadowVisibility(shadowtex0, shadowtex1, shadowcolor0, sp, angle);
        
        float fade = saturate(1.0 - length(worldPos.xz) / shadowDistance);
        shadowVis = mix(vec3(1.0), shadowVis, fade);
    }
#endif
    ...
    Material m;
    m.albedo      = albedo;
    m.normalW     = normalize(worldNormal);
    m.lm          = lmcoord;
    m.smoothness  = 0.0;
    m.reflectance = 0.0;
    m.emissive    = 0.0;
